...
def load_train_data(filename, n_train, n_train_each, bond_label,bond_data, current_class):
    ...
    data_group = np.zeros((16, 16, 10, image_group.shape[3], bond_data), dtype=np.float64)
    ...
    for k, l, m in product(range(bond_data), range(9), range(int(n_train_each / 9))):
        train_data[:, :, k, n_train_each+l * int(n_train_each/9) + m] = data_group[:, :, list(rest)[l], m, k]
    ...
    for i, j in product(range(16), range(16)):
        data_tensor[i][j] = tn.Tensor(train_data[i, j, :, :], labels=["up", "down"])
...
def load_test_data(filename, n_test, n_test_each, bond_data):
    ...
    for i, j in product(range(16), range(16)):
        test_tensor[i][j] = tn.Tensor(test_data[i, j, :, :], labels=["up","down"])
