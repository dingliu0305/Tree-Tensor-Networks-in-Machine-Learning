...
# data_folder = "/export/Tree_Tensor_TSNE/tncontract_master/data/"
...
# load training data
print("loading data")
input= open('/root/PycharmProjects/TN_image/TN_MINIST/tsne.pkl', 'rb')
...
def tsne(contraction, which_layer, per, lear, tags=LAB):
    ...
    mf = TSNE(n_components=2, perplexity=per, learning_rate=lear, init='pca', n_iter=1200)
...
def plot(contraction,  which_layer, per, lear, tags=LAB):
    ...
    x=result[:,0]
    y=result[:,1]
    ax1.scatter(x[0:n_train_each], y[0:n_train_each], s=11, c='b', marker="o", label='Planes',alpha=0.5)
    ax1.scatter(x[n_train_each+1:n_train], y[n_train_each+1:n_train], s=11, c='r', marker="o", label='Horses',alpha=0.5)
...
def sweep(contraction, which_layer, per, tags=LAB):
    ...
    for i in range(0,len(L)):
        f=plotplot(contraction, i, per, L[i], tags=LAB)
